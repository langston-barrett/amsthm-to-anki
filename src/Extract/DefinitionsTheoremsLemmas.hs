...
import safe Text.LaTeX
import safe Text.LaTeX.Base.Syntax

-- My modules
import safe Extract.Util
import safe Types
...
-- | Transferms a block representing a "definition/lemma/theorem" environment
...
-- argument.
makeEnvNote :: (LaTeX -> LaTeX -> Notecard)
            -> ([TeXArg], LaTeX)
            -> Either Error Notecard
makeEnvNote makeSpecificNote (args, content) =
  case args of
    ...
    _ ->
      Left
        (UserError $
         "couldn't find title of this environment:\n" ++ render content)

definitionsTheoremsLemmas :: LaTeX -> ([Error], [Notecard])
definitionsTheoremsLemmas srcTeX =
  partitionEithers $
  map (makeEnvNote makeDefinitionNote) (lookForEnvStar "definition" srcTeX) ++
  map (makeEnvNote makeTheoremNote) (lookForEnvStar "theorem" srcTeX) ++
  map (makeEnvNote makeLemmaNote) (lookForEnvStar "lemma" srcTeX)
  where
