...
testMakeEnvNote = testGroup "makeEnvNote" $
  let errorsOn tex = assertLeft (makeEnvNote (makeAuxNote "" "") tex)
  in
  [ HU.testCase "fails on SymArg" $ errorsOn ([SymArg (TeXRaw "")], mempty)
...
testDefinitionsTheoremsLemmas = testGroup "definitionsTheoremsLemmas" $
...
tests ∷ TestTree
tests = testGroup "" [ testLookForEnv
                     , testMakeEnvNote
